...
const DividerStyled = styled.div`
  width: ${(props) =>
    `${props.divWidth?.size || 100}${props.divWidth?.unit || "%"}`};
  position: relative;
  ${(props) =>
    props.align === "center"
      ? `
  ...
  ${(props) =>
    props.width_tablet?.size
      ? `
        @media ${device.tablet} {
            width: ${props.width_tablet?.size}${props.width_tablet?.unit};
            ${
              props.align_tablet === "center"
                ? `margin: 0 auto;`
                : `float: ${props.align_tablet};`
            }
        }`
      : ""}

  ${(props) =>
    props.width_mobile?.size
      ? `
        @media ${device.mobile} {
            width: ${props.width_mobile?.size}${props.width_mobile?.unit};
            ${
              props.align_mobile === "center"
                ? `margin: 0 auto;`
                : `float: ${props.align_mobile};`
            }
        }`
      : ""}

  ${(props) => `&:before${
    props.look === "line_text" || props.look === "line_icon" ? ", &:after" : ""
  } {
    content: "";
    width: 100%;
    ${
      ["solid", "double", "dotted", "dashed"].includes(props.styleBorder)
        ? `border-top: ${props.weight.size}${props.weight.unit} ${
            props.styleBorder
          } ${props.borderColor || "#54595F"};`
        : `-webkit-mask-size: ${
            props.styleBorder.indexOf("tribal") >= 0
              ? "auto"
              : props.patternSize.size + "" + props.patternSize.unit
          } 100%;
            -webkit-mask-repeat: ${
              props.styleBorder.indexOf("tribal") >= 0 ? "repeat-x" : "round"
            };
            -webkit-mask-image: ${customDividerStyle(
              props.weight.size,
              props.styleBorder
            )};
            background-color: ${props.borderColor || "#54595F"};
            min-height: ${props.patternHeight?.size || 20}${
            props.patternHeight?.unit || "px"
          };
    `
    }
  }`}
  ...
  return (
    <DividerWrapper>
      <DividerStyled
        ...
        look={look}
      >
        {look === "line_text" && <IconTextContainer>{text}</IconTextContainer>}
        {look === "line_icon" && (
          <IconTextContainer>
            <FontAwesomeIcon
              icon={getIcon(icon.library, icon.value.split(" ")[1])}
            />
          </IconTextContainer>
        )}
      </DividerStyled>
